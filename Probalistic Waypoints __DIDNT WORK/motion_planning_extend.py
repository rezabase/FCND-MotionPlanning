...
from planning_utils import read_home_location  #, new_global_position, prune_path, find_start_goal
...
from planning_utils_extend import create_graph, find_nearest_waypoint, a_star_waypoints, heuristic
...
class MotionPlanning(Drone):
    ...
    def plan_path(self):
        ...
        print("\nCurrent nx version: {0}, it must be 2.1".format(nx.__version__)) # should be 2.1 
        ...
        nearest_node_index = find_nearest_waypoint(current_local_position, graph_nodes_list)
        start_node = graph_nodes_list[graph_nodes_list]
        
        nearest_node_index = find_nearest_waypoint(goal_local_position, graph_nodes_list)
        goal_node = graph_nodes_list[nearest_node_index]
        print("Start Node: " + str(start_node))
        print("End Node: " + str(goal_node))

        self.waypoints, _ = a_star_waypoints(graph, heuristic, start_node, goal_node)

        #Adding my actual start and end positions to the begining and the end of the waypoints_path
        self.waypoints.insert(0, goal_local_position) #adding the start world position 
