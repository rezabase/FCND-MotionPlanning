...
2. In the local_position_callback(), i increased the minimum distance to each waypoint, to trigger waypoint_transision towards the nest waypoint. I increased it from 1.0 to 6.0. that has made the drone transition between waypoints much smoother and it looks better.   
...
And there are the steps I took to create it: 

1. I created a 3D grid around the position of the drone that looks like the following: 
...
3. I did really not know what to use to calculate the intermediate path of the drone within the 3D grid, so I modified the A* algorythm to find a path within the 3D grid using the vector costs that I calculated using the potential_field code. picture below shows the path. (please note that vectors are pointing to the opposite direction)

![alt text](misc/rh3.png "Step 3")

Anyway, if you run the code, you will see that the drone does strange things sometimes. One problem might be that I havent done the 3D grid calculations entirly correctly and the obstacles are posisioned in the wrong places in the 3D grid. Thats something to investigate next...
...
There is no calculate_box() anymore because this time we will be following the waypoints that were created during the planning phase instead of flying within a box. 
4. arming_transition() doesn’t set the home position anymore because this time we will be landing at a destination location.
...
6. plan_path() function is incomplete and needs the planning code that is part of this assignment. 


#### To summarise, the code does the following: 

1. Connects to the simulator using MavlinkConnect()
...
3. The state_callback method initiates ARMING, and PLANNING phases. 
4. Plan_path() method, loads the 2.5D obstacle list (colliders.csv). it converts it to usable data, calculates the planning path and creates the waypoints that the drone will follow. 
5. positioning events call the local_position_callback() method periodically and it will detect when the drone reaches  intermediate drone destinations until the last waypoint. After reaching the last waypoint, it will initiate the landing.
...
#### 2c) In the starter code, the start point for planning is hardcoded as map centre. Change this to be your current local position.
...
#### 2d) In the starter code, the goal position is hardcoded as some location 10 m north and 10 m east of map centre. Modify this to be set as some arbitrary position on the grid given any geodetic coordinates (latitude, longitude)

Added the following code to:
1. goal_global_position = is a global latitude, longitude position. 
2. First, I made a method that added some distance to the home_location to use it as the goal but then I realised its easier to fly the drone manually to some other location and type in the locations in the code manually. Anyway, if needed, the following line is still there and can be used in future..
...
Added a collinearity check and prune_path method to prune some of the zick-zack going waypoints. 

I prepared both the np.linalg.det() that supports floating function and the option that supports only int values. I guess, because of the CPU cost of floating point calculation, I should try to use the int option. 

As there were many waypoints within 2-3 meters zickzacking on a straight path, I used a big epsilon (epsilon=2.5) trying to reduce the number of waypoints. 
...
Its so exciting to see that the drone flows the path all the way to the end. 
