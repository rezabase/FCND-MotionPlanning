...
from receding_horizon import HorisonMap3D, potential_field, a_star_voxmap
...
class MotionPlanning(Drone):
    ...
    def waypoint_transition(self):
        ...
        self.next_waypoint = self.waypoints.pop(0) #Saving the next waypoint in another variable becouse the target_position could be changed by the receding_horison transision in case there are obstacles on the way.
    ...
    def plan_path(self):
        ...
        self.collider_map_data = np.loadtxt('colliders.csv', delimiter=',', dtype='Float64', skiprows=2)
        
        # Define a grid for a particular altitude and safety margin around obstacles
        grid, north_offset, east_offset = create_grid(self.collider_map_data, TARGET_ALTITUDE, SAFETY_DISTANCE)
    ...
    def receding_horizon_check(self):

        #If there is no next destination, no need to continue
        #I'm assuming that take off and landing are obstacle free. 
        if self.next_waypoint == None: 
            return 

        #Update the Voxmap based on the current position
        #Note: here I need to add the sensor information to the Voxmap.
        myvoxmap = self.grid3D.create_voxmap(self.collider_map_data, self.local_position)

        if not self.grid3D.has_obstacle(): #if there are no obstacles detected in the 3D horizon grid, dont need to do anything
            self.target_position = self.next_waypoint
            self.cmd_position(self.target_position[0], self.target_position[1], self.target_position[2], self.target_position[3])
            return
        ...
        #REMEMBER IMPORTANT: the goal position below falls OUTSIDE the actuall grid3D voxmap but since we are only calculating distance cost, its fine 
        goal = self.grid3D.map3dgrid_from_worldpos(self.next_waypoint[:3]) #target_position is now sett to the next waypoint location. 
        print("\n**  Goal position in grid3D.map3dgrid grids: " + str(goal))
        print("*  Voxmap shape: " + str(self.grid3D.voxmap.shape))

        field_costs = potential_field(self.grid3D.voxmap, goal, alpha, beta, q_max)

        start_pos = self.grid3D.map3dgrid_from_worldpos(self.local_position)
        print("*  start_pos"+str(start_pos))
        intermediate_path, _ = a_star_voxmap(self.grid3D.voxmap, start_pos, self.next_waypoint[:3], field_costs)

        item_in_path = 1 #int(len(intermediate_path)-1)
        print("*  intermediate_path[item_in_path]: {0}\n".format(np.array(intermediate_path)[item_in_path])) 
        next_target = self.grid3D.worldpos_from_map3dgrid(intermediate_path[item_in_path]) #just took one of the items from the path
        print("\n\n **** Obstacles found in 3D grid, re-routing to: {0}\n".format(next_target))
        print("*  current position: {0}\n".format(self.grid3D.robot_world_position))
        print('*  target position', next_target)
        self.target_position = [next_target[0], next_target[1], next_target[2], self.target_position[3]]
        self.cmd_position(self.target_position[0], self.target_position[1], self.target_position[2], self.target_position[3])
